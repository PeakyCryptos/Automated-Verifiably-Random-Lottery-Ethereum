...
contract automatedLottery is VRFConsumerBaseV2, KeeperCompatibleInterface {
    ...
    // minimum dollars to enter lottery
    uint256 public immutable minimumDollars;
     ...
     *
     *  interval is time in seconds to execute
     */
    uint256 public immutable interval;
    uint256 public timeKeeper;
    //------------------------------------------------------------------
    ...
    function enter() external payable {
        ...
        // (amt ** price) / 10^26
        uint256 dollarsPassed = (msg.value * getLatestPrice()) / 10**26;
        // min entry $10
     ...
     */
    function viewPlayers() external view returns (address[] memory) {
    ...
    function pickWinner() internal {
         ...
         */
        uint256 playerOneIndex = randomResult[0] % totalPlayers;
        uint256 playerTwoIndex = randomResult[1] % totalPlayers;
        ...
        uint256 weight = playerOne.weight + playerTwo.weight;
         ...
         */
        uint256 settler = randomResult[2] % weight;
    ...
    {
        if (timeKeeper != 0 && (block.timestamp - timeKeeper) > interval) {
            upkeepNeeded = true;
        } else {
            upkeepNeeded = false;
        }
